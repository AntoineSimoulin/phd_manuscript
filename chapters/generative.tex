...
\midrule
\midrule
WikiText-35-FR or WikiText-72-FR (ppl) & 109.2 & \textbf{12.9} \\
\bottomrule
\end{tabularx}}
...
We also considered a language models with 5-grams and kneser-ney smoothing \parencite{ney_94} using the SRILM tool \parencite{stolcke_02} as baseline. The approaches are not directly comparable because the tokenization is different and our model is trained on a much larger volume of data. But the model may be used to assess the distinction between the two benchmarks WikiText-35-FR and WikiText-72-FR.
...
\bottomrule
\end{tabularx}}
\caption{\labtab{generative:lm-scores}Perplexity of our models. We train the $n$-gram model on the respective training set. The perplexity is directly measured on the test set which are identical for two benchmarks.}
\end{table*}
