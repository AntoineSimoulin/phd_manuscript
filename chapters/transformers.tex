...
Then an \textit{update} step (\refeq{update}) that update each hidden embedding given its neigborhood aggregated information and its previous state.

\begin{align}
    m^{(k)}_{\mathcal{N}(u)} &= \text{aggregate}^{(k)}\left(\{h_v^{(k)}, \forall v \in \mathcal{N}(u)\}\right), \labeq{aggregate} \\
    h^{(k+1)}_u &= \text{update}^{(k)}\left(h^{(k)}_u, m^{(k)}_{\mathcal{N}(u)}\right), \labeq{update}
\end{align}

Here, the $\text{aggregate}$\sidenote{The $\text{aggregate}$ function takes a set as input and is therefore permutation invariant by design} and $\text{update}$ function are differentiables and $m_{\mathcal{N}(u)}$ is the \textit{message} aggregated from $u$ neigborhood.
...
\begin{equation}
    h^{(k+1)}_u = \text{aggregate}^{(k)}\left(\{h_v^{(k)}, \forall v \in \mathcal{N}(u) \cup \{u\}\}\right), \labeq{update-aggregate}\\
\end{equation}

In \refeq{embeddings} defines the embedding of each node as its hidden state after $K$ message passing iterations:
...
We can adapt the \refeq{update-aggregate} for transformers as follow\sidenote{In fact, we can also formally decompose transformers layers by considering the MHA layer as the $\text{aggregate}$ function and the FFN layer as the $\text{update}$ function. This interpretation may provide a formal backgroound to analyze the role of the FFN.}:
...
Commonly this issue is adressed by concatenating the aggregated state with the previous hidden state before performing the update state.
This skip connection aim at explicitely preserving information from the previous iteration during the update.

Another common pitfall of graph neural networks is that every nodes may have a different degree. 
This may affect the scale of the vector during agregation.
This variance in scale may results in numerical and optimization instability.
A common way to ableviate this issue is to normalize the operation based upon the degree of nodes involved.
The normalization is also a sensitive opertations of transformers (see csordas)

This formalization account for the linear order of words encoded in the positional embedding layer $W_p$.
...
In the graph neural network perspective, transformers progressively refine the feature through an iterative process.
As described in \textcite{xin_20}.  become more fine-grained at each iteration.
This also provide a new interpreation path for \textsc{Albert} \parencite{lan_20}.
The model is based on the transformer architecture, except that weights are tied across layers.
From a graph neural netwrok perspetive, the model use the same message passing function at each iteration.
...
In this case, the $\text{aggregate}$ function consists on the simple sum of the node neigborhood hidden states.
...
The number of number of edges $|\mathcal{E}|$ is equalt to the number of nodes squarred $|\mathcal{V}|^2$.
...
We hypothesize that node hidden states will eventually converge toward a stable point.
We also hypothesize that some nodes require more iteration than others and that this convergence process will depend from the word and its context.
